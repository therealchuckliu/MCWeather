...
The model that was implemented is a second order Hidden Markov Model. Given the data available, our model does not require an expectation maximization framework to estimate the emissions probabilities or transition probabilities. Instead, to model the emissions probabilities, we modeled each of the observations as either a Gaussian distribution or a Log-normal distribution. We inferred the corresponding distribution parameters from the historical data available. We then constructed a multivariate Gaussian representing the probability of a vector of observations corresponding to a particular hidden state. To model the transition probabilities, we simply counted the state transitions between the discrete weather events available in our labeled data. We also used a naive estimate of the initial state probability for our model. 

To infer the weather events on each day over our test period, we ran an extended Viterbi algorithm to infer the most probable sequence of hidden states given the observations over the five year test period. Using the recursive tracing approach, introduced by He, to evaluate the maximum a posteriori estimation of the second order HMM makes it easy to extend to higher order models for improved inference accuracy.      
...
Over the five year test period, the model had an overall inference accuracy of over 65\%. Specifically, the model did well to infer extreme events like thunderstorms and specific events like Fog days as can been seen in the figure below. The model did less well on highly overlapping events like Rain, Rain-Snow, Fog-Snow etc. However, still considerably better than a naïve baseline. Furthermore, even for the worst performing states, from an end-user standpoint, this isn't as bad as it seems. In this problem, there is an inherent ordering of the hidden states, with some states being closer to each other than others. For example,  Rain-Snow, Rain and Fog-Rain-Snow are more similar to each other than to Fog days or Normal days which implies that such a misclassification is less impactful than would seem initially. The confusion matrix indicates the same and for the sake of brevity is discussed in the notebook.  
